...
Метрики вычисляются evaluation harness (`tools/evaluation/`) и предназначены для UI.
...
1. Simulator применяет saturation к CPM, CTR и новому охвату, но не к CR. Planner намеренно добавляет
   CR fatigue с коэффициентом `0.5` от CTR exponent, поэтому это policy benchmark, а не точная копия
   Simulator.
