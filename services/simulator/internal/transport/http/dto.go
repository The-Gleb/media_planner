...
func (d simulationConfigDTO) domain() (domain.SimulationConfig, error) {
	...
	for _, e := range d.ScenarioEvents {
		events = append(events, domain.ScenarioEvent{ChannelID: e.ChannelID, Metric: e.Metric, StartIndex: e.StartIndex, DurationHours: e.DurationHours, Multiplier: e.Multiplier})
	}
