...
import type { Allocation, ExpectedOutcome, HourlyExpected, KPI, MediaPlan, PlanResult, TargetKPI } from '../domain/planning'
...
const decimal = (v:unknown):v is string => typeof v==='string'&&DECIMAL.test(v)
...
function decodeHourlyExpected(raw:unknown):HourlyExpected|null { if(raw===null) return null; const value=object(raw,'hourly_expected');exact(value,['spend','impressions','unique_reach','clicks','conversions'],'hourly_expected');if(!money6(value.spend)||!decimal(value.impressions)||!decimal(value.unique_reach)||!decimal(value.clicks)||!decimal(value.conversions))throw new Error('invalid_hourly_expected');return{spend:value.spend,impressions:value.impressions,uniqueReach:value.unique_reach,clicks:value.clicks,conversions:value.conversions} }
function decodeAllocations(raw:unknown):Allocation[]{if(!Array.isArray(raw)||raw.length<1||raw.length>43200)throw new Error('invalid_plan_allocations');const allocations=raw.map((rawItem)=>{const item=object(rawItem,'plan_allocation');exact(item,['channel_id','hour','budget_cap','expected'],'plan_allocation');if(typeof item.channel_id!=='string'||!CHANNEL.test(item.channel_id)||!integer(item.hour,0,2159)||!money6(item.budget_cap))throw new Error('invalid_plan_allocation');return{channelId:item.channel_id,hour:item.hour as number,budgetCap:item.budget_cap,expected:decodeHourlyExpected(item.expected)}});for(let i=1;i<allocations.length;i++){const a=allocations[i-1],b=allocations[i];if(a.hour>b.hour||(a.hour===b.hour&&a.channelId>=b.channelId))throw new Error('invalid_plan_order')}return allocations}
...
export function decodePlanResult(raw: unknown): PlanResult {
  ...
  if(value.type==='fixed_budget'){
    if(value.required_budget!==null||value.target!==null)throw new Error('invalid_fixed_plan')
    return {...common,planId:value.plan_id,feasible:true,type:'fixed_budget',budget:value.budget,unallocatedBudget:value.unallocated_budget,expected:value.expected===null?null:decodeExpected(value.expected),allocations,requiredBudget:null,reason:null,target:null}
  }
