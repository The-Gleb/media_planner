...
const KEYS = ['request_id','state_revision','plan_id','feasible','type','strategy','optimize','currency','budget','horizon','expected','allocations','required_budget','reason','target']
...
export function decodePlanResult(raw: unknown): PlanResult {
  ...
  if(value.feasible===false){
    ...
    return {...common,stateRevision:0,planId:null,feasible:false,type:'target_kpi',strategy:'optimized',budget:null,expected:decodeExpected(value.expected),allocations:[],requiredBudget:null,target:decodeTarget(value.target),reason:{code:'target_exceeds_capacity',detail:reason.detail,maxAchievable:reason.max_achievable,recommendedTarget:reason.recommended_target}}
  }
  if(value.feasible!==true||typeof value.plan_id!=='string'||!/^[a-f0-9]{64}$/.test(value.plan_id)||!money6(value.budget)||value.reason!==null)throw new Error('invalid_plan')
  ...
  if(value.type==='fixed_budget'){
    ...
    return {...common,planId:value.plan_id,feasible:true,type:'fixed_budget',budget:value.budget,expected:null,allocations,requiredBudget:null,reason:null,target:null}
  }
  ...
  return {...common,planId:value.plan_id,feasible:true,type:'target_kpi',strategy:'optimized',budget:value.budget,expected:decodeExpected(value.expected),allocations,requiredBudget:value.required_budget,reason:null,target:decodeTarget(value.target)}
}
