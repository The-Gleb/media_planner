...
export function RunProgress({ session, status }: { session: ActiveRun; status: ExecutionStatus }) {
  ...
  return <div className="run-progress" aria-live={status === 'running' ? 'off' : 'polite'}>
    ...
    <label htmlFor="campaign-progress">Прогресс: {completed} выполнено, {session.remainingHours} осталось, {percent.toFixed(1)}% · час {completed} из {session.durationHours}</label>
