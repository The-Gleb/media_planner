...
describe('audience launch', () => {
  it('sends the selected segments to the simulator and freezes them in the active draft', async () => {
    ...
    draft.simulation.audience = audience
    ...
    expect(result.current.state.activeDraft?.simulation.audience).toEqual(audience)
    act(() => result.current.dispatch({ type: 'draft', draft: { ...draft, simulation: { ...draft.simulation, audience: undefined } } }))
    expect(result.current.state.activeDraft?.simulation.audience).toEqual(audience)
  })
})
