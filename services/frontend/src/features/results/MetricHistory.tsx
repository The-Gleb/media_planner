...
import { chartRows, seriesDefinitions } from './metricSeries'
import { MetricChart } from './MetricChart'
import { ExactValueInspector } from './ExactValueInspector'

export function MetricHistory({ history, channelIds, currency, running }: { history: HourlyResult[]; channelIds: string[]; currency: string; running: boolean }) {
  ...
  const unit = metric === 'spend' || metric === 'ecpm' ? currency : 'за час'
  return <section className="card" aria-labelledby="history-title">
    <h2 id="history-title">История наблюдений</h2>
