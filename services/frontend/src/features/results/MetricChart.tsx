...
const tickLabel = cachedFormatter((v: unknown) => new Date(String(v)).toLocaleDateString('ru-RU', { day: '2-digit', month: '2-digit', hour: '2-digit' }))
const tooltipLabel = cachedFormatter((v: unknown) => new Date(String(v)).toLocaleString('ru-RU'))
  ...
  return <div className="chart-wrap" role="img" aria-label={`${title}. Единица: ${unit}. Точные значения доступны ниже.`}>
    <ResponsiveContainer width="100%" height="100%">
      <LineChart data={rows} accessibilityLayer margin={{ top: 16, right: 20, bottom: 18, left: 10 }}>
        ...
        <XAxis dataKey="hour" tickFormatter={tickLabel} minTickGap={28} />
        <YAxis width={82} />
